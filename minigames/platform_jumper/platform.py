...
def create_random_platform(last_platform=None, base_y=None):
    ...
    if last_platform:
        ...
        # Расчёт горизонтальной позиции с учётом допустимого диапазона
        min_x = max(PLATFORM_WIDTH // 2, last_platform.center_x - HORIZONTAL_RANGE)
        max_x = min(
            WIDTH - PLATFORM_WIDTH // 2,
            last_platform.center_x + HORIZONTAL_RANGE,
        )
